...
if (! function_exists('settings')) {
    function settings($user_id = null, $store_id = null)
    {
        ...
        if (is_null($store_id) && is_null($user_id) && request()->attributes->has('resolved_store')) {
            $resolvedStore = request()->attributes->get('resolved_store');
            $store_id = $resolvedStore->id;
            $user_id = $resolvedStore->user_id;
        }
        ...
        if ($store_id !== null) {
            $storeSettings = Setting::where('user_id', $user_id)
                         ->where('store_id', $store_id)
                         ->pluck('value', 'key')
                         ->toArray();
            
            return array_merge($globalSettings, $storeSettings);
        }
...
if (! function_exists('getSetting')) {
    function getSetting($key, $default = null, $user_id = null, $store_id = null)
    {
        ...
        // For store-specific settings, use method with fallback to user global
        if ($store_id !== null) {
            return \App\Models\Setting::getSetting($key, $user_id, $store_id, $default, true);
        }
...
if (! function_exists('processPaymentSuccess')) {
    function processPaymentSuccess($data)
    {
        ...
        if (isset($data['payment_id'])) {
            $planOrder = \App\Models\PlanOrder::where('payment_id', $data['payment_id'])->first();
        } else {
            $planOrder = null;
        }
    ...
    function reactivateResources($user)
    {
        ...
        foreach ($user->stores as $store) {
            ...
            foreach ($deactivatedUsers as $storeUser) {
                \App\Models\User::where('id', $storeUser->id)->update(['status' => 'active']);
            }
        }
        ...
        if ($maxProductsPerStore > 0) {
            foreach ($user->stores as $store) {
                ...
                foreach ($deactivatedProducts as $product) {
                    \App\Models\Product::where('id', $product->id)->update(['is_active' => true]);
                }
            }
        }
    }
        ...
        foreach ($user->stores as $store) {
            ...
            if ($storeUsers->count() > $maxUsersPerStore) {
                ...
                foreach ($usersToDeactivate as $storeUser) {
                    \App\Models\User::where('id', $storeUser->id)->update(['status' => 'inactive']);
                }
            }
        }
            ...
            foreach ($user->stores as $store) {
                ...
                if ($products->count() > $maxProductsPerStore) {
                    ...
                    foreach ($productsToDeactivate as $product) {
                        \App\Models\Product::where('id', $product->id)->update(['is_active' => false]);
                    }
                }
            }
        }
