    ...
    public function export()
    {
        ...
        foreach ($coupons as $coupon) {
            $csvData[] = [
                ...
                $coupon->code,
                ucfirst($coupon->type),
                $coupon->type === 'percentage' ? $coupon->discount_amount . '%' : formatStoreCurrency($coupon->discount_amount, $user->creatorId(), $currentStoreId),
                $coupon->minimum_spend ? formatStoreCurrency($coupon->minimum_spend, $user->creatorId(), $currentStoreId) : 'No minimum',
                $coupon->maximum_spend ? formatStoreCurrency($coupon->maximum_spend, $user->creatorId(), $currentStoreId) : 'No maximum',
