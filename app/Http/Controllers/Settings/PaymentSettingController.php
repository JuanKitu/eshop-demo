...
class PaymentSettingController extends Controller
{
    public function index()
    {
        ...
        $messagingOrderVariables = PaymentSetting::where('user_id', $ownerId)
            ->where('store_id', $storeId)
            ->where('key', 'messaging_order_variables')
            ->first();
            
        $messagingItemVariables = PaymentSetting::where('user_id', $ownerId)
            ->where('store_id', $storeId)
    ...
    public function store(Request $request)
    {
        ...
        // WhatsApp and Telegram settings only for store owners/staff
    ...
    private function savePaymentSettings(array $settings): void
    {
        ...
        // Add COD, WhatsApp and Telegram settings only for company users (store checkout)
        if ($user->type !== 'superadmin') {
            $settings['is_cod_enabled'] = request()->boolean('is_cod_enabled');
    ...
    public function getEnabledMethods()
    {
        ...
        if ($user->type !== 'superadmin') {
            ...
            $enabledMethods = getEnabledPaymentMethods($user->creatorId(), $storeId);
        } else {
