...
class ProductController extends BaseController
{
    ...
    public function index()
    {
        ...
        $lowStockThreshold = \App\Models\Setting::getSetting('low_stock_threshold', $user->creatorId(), $currentStoreId, 20);
    ...
    public function show(string $id)
    {
        ...
        $stats['formatted_revenue'] = formatStoreCurrency($stats['revenue'], $user->creatorId(), $currentStoreId);
    ...
    public function export()
    {
        ...
        foreach ($products as $product) {
            ...
            if ($product->variants && is_array($product->variants) && count($product->variants) > 0) {
                ...
                foreach ($product->variants as $variant) {
                    if (is_array($variant) && isset($variant['name'])) {
                        $variantList[] = $variant['name'] . (isset($variant['price']) ? ' (' . formatStoreCurrency($variant['price'], $user->creatorId(), $currentStoreId) . ')' : '');
                    }
                }
            ...
            $csvData[] = [
                ...
                formatStoreCurrency($product->price, $user->creatorId(), $currentStoreId),
                $product->sale_price ? formatStoreCurrency($product->sale_price, $user->creatorId(), $currentStoreId) : 'Not set',
                $product->stock,
                $variantDetails,
