...
class ThemeController extends Controller
{
    ...
    protected function getStore(Request $request, $storeSlug = null)
    {
        ...
        if ($request && $request->attributes->has('resolved_store')) {
            ...
            return [
                ...
                'theme' => $store->theme,
                'slug' => $store->slug,
                'dynamicTitleText' => getSetting('titleText', $store->name),
        ...
        if (!$storeSlug && $request->has('orderNumber')) {
             $order = Order::where('order_number', $request->orderNumber)->first();
             if ($order) {
                 $storeSlug = $order->store->slug;
             }
        } elseif (!$storeSlug) {
             // Try to get orderNumber from route
    ...
    public function cart(Request $request)
    {
        ...
        $cartItems = CartItem::where('store_id', $store['id'])
            ...
            ->get()
            ->map(function ($item) {
                $selectedVariants = is_string($item->variants) ? json_decode($item->variants, true) : $item->variants;
                return [
                    'id' => $item->id,
                    'product_id' => $item->product_id,
                    'name' => $item->product->name,
                    'price' => $item->product->getVariantOriginalPrice($selectedVariants),
                    'sale_price' => $item->product->getVariantPrice($selectedVariants),
    ...
    public function myProfile(Request $request)
    {
        ...
        // Get wishlist count
        $wishlistCount = \App\Models\WishlistItem::where('customer_id', $customer->id)->count();

        return Inertia::render($profilePage, array_merge($this->getCommonData(), [
            'user' => $user,
            'store' => $store,
            'theme' => $store['theme'],
            'storeContent' => $storeContent,
        ...
        $relatedProducts = Product::where('store_id', $store['id'])
            ...
            ->map(function ($relatedProduct) {
                return [
                    ...
                    'variant_options' => $relatedProduct->variant_options,
                    'category' => [
    ...
    public function blogPost(Request $request)
    {
        ...
        return Inertia::render($blogPostPage, array_merge([
            ...
            'post' => [
                ...
                'index_in_search' => $post->index_in_search,
            ],
            'customPages' => $customPages,
            'cartCount' => 3,
            'wishlistCount' => 5,
