...
class POSController extends BaseController
{
        ...
        $products = Product::where('store_id', $currentStoreId)
            ...
            ->map(function ($product) {
                ...
                return [
                    ...
                    'variants' => $variants,
                    'variant_options' => is_string($product->variant_options) ? json_decode($product->variant_options, true) : $product->variant_options,
    ...
    public function receipt($id)
    {
        ...
        $settings = array_merge($posSettings->toArray(), [
            ...
            'currencySymbolSpace' => $storeSettings['currencySymbolSpace'] ?? false,
            'currencySymbolPosition' => $storeSettings['currencySymbolPosition'] ?? 'before',
            'companyName' => $storeSettings['titleText'] ?? $storeSettings['company_name'] ?? 'My Store',
        ]);
        
        return Inertia::render('pos/receipt', [
            'transaction' => $transaction,
            'settings' => $settings,
            'storeLogo' => $storeSettings['logoDark'] ?? null,
        ]);
    }
    ...
    public function transactions()
    {
        ...
        $stats['formatted_todaySales'] = formatStoreCurrency($stats['todaySales'], $user->creatorId(), $currentStoreId);
        $stats['formatted_weekSales'] = formatStoreCurrency($stats['weekSales'], $user->creatorId(), $currentStoreId);
        $stats['formatted_averageSale'] = formatStoreCurrency($stats['averageSale'], $user->creatorId(), $currentStoreId);
        $stats['formatted_refundAmount'] = formatStoreCurrency($stats['refundAmount'], $user->creatorId(), $currentStoreId);
    ...
    public function updateSettings(Request $request)
    {
        ...
        // Update all settings from request (currency is NOT stored in pos_settings, comes from Store Settings)
        $settings->fill($request->except('currency'));
        $settings->save();
        
        return redirect()->back()->with('success', __('POS settings updated successfully.'));
    }
    ...
    public function export()
    {
        ...
        $transactions = POSTransaction::with(['customer', 'cashier', 'items'])
            ->where('store_id', $currentStoreId)
            ->orderBy('created_at', 'desc')
            ->get();
        
        $csvData = [];
        $csvData[] = ['Transaction Number', 'Date', 'Time', 'Customer', 'Cashier', 'Items Count', 'Subtotal', 'Tax', 'Discount', 'Total', 'Status'];
        ...
        foreach ($transactions as $transaction) {
            $csvData[] = [
                ...
                $transaction->customer ? 
                    $transaction->customer->first_name . ' ' . $transaction->customer->last_name : 
                    'Walk-in Customer',
                $transaction->cashier ? $transaction->cashier->name : 'Unknown',
                $transaction->items->sum('quantity'),
                formatStoreCurrency($transaction->subtotal, $user->creatorId(), $currentStoreId),
                formatStoreCurrency($transaction->tax, $user->creatorId(), $currentStoreId),
                formatStoreCurrency($transaction->discount, $user->creatorId(), $currentStoreId),
                formatStoreCurrency($transaction->total, $user->creatorId(), $currentStoreId),
                ucfirst($transaction->status)
            ];
        }
        
        $filename = 'pos-transactions-export-' . now()->format('Y-m-d') . '.csv';
