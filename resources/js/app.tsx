...
createInertiaApp({
    title: (title) => {
        // Get dynamic title text from shared props or store props
        const dynamicTitle = (window as any).page?.props?.dynamicTitleText || (window as any).page?.props?.store?.dynamicTitleText || appName;
    ...
    setup({ el, App, props }) {
        ...
        if (Object.keys(globalSettings).length > 0) {
            ...
            const dynamicTitle = pageProps.dynamicTitleText || pageProps.store?.dynamicTitleText || (globalSettings as any).titleText || appName;
        ...
        const resolveComponent = props.resolveComponent;
                        ...
                        <SidebarProvider>
                            <BrandProvider globalSettings={currentGlobalSettings} user={user}>
                                <Suspense fallback={<div className="flex h-screen w-full items-center justify-center">Loading...</div>}>
                                    <App {...appProps} resolveComponent={resolveComponent} />
                                </Suspense>
                                <CustomToast />
                            </BrandProvider>
                        </SidebarProvider>
        ...
        router.on('navigate', (event) => {
            try {
                ...
                if (Object.keys(updatedGlobalSettings).length > 0) {
                    ...
                    const dynamicTitle = pageProps.dynamicTitleText || pageProps.store?.dynamicTitleText || (updatedGlobalSettings as any).titleText || appName;
                    document.title = dynamicTitle;
                }
