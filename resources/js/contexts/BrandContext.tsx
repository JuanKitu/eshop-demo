...
export function BrandProvider({ children, globalSettings, user }: { children: ReactNode; globalSettings?: any; user?: any }) {
  ...
  const getEffectiveSettings = () => {
    ...
    // Check if the current path is the landing page or a known public route
    // The app is hosted in /product/storego/storego-saas/main_file/ so we check if the path ends with /main_file or /main_file/
    const isPublicRoute = 
      currentPath === '/' || 
      currentPath.endsWith('/main_file') ||
      currentPath.endsWith('/main_file/') ||
