...
export default function EditStore({ store, availableThemes, planPermissions, serverIp }: EditStoreProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    email: store?.email || '',
    theme: store?.theme || 'home-accessories',
    ...
    pwa_short_name: store?.pwa_short_name || store?.name?.substring(0, 12) || '',
    pwa_description: store?.pwa_description || store?.description || '',
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      {t('URL slug for your store. Must be unique and contain only lowercase letters, numbers, and hyphens.')}
                    </p>
                  </div>
                </div>
                <div>
