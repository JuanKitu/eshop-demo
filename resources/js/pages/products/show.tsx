...
export default function ShowProduct() {
  ...
  return (
    <PageTemplate
      ...
      description={t('View full details and pricing for product: {{name}}', { name: product.name })}
      url="/products/show"
      actions={pageActions}
      breadcrumbs={[
    ...
    >
      <div className="space-y-6">
        ...
        {product.variant_options && product.variant_options.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle>{t('Variant Pricing')}</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="relative overflow-x-auto border rounded-lg">
                <table className="w-full text-sm text-left">
                  <thead className="text-xs uppercase bg-muted">
                    <tr>
                      <th className="px-6 py-3">{t('Variant')}</th>
                      <th className="px-6 py-3">{t('Price')}</th>
                      <th className="px-6 py-3">{t('Sale Price')}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {product.variant_options.map((option: any, index: number) => (
                      <tr key={index}>
                        <td className="px-6 py-4 font-medium">{option.name || option.variant}</td>
                        <td className="px-6 py-4">{formatCurrency(option.price)}</td>
                        <td className="px-6 py-4">
                          {option.sale_price ? (
                            <span className="text-green-600 font-semibold">{formatCurrency(option.sale_price)}</span>
                          ) : (
