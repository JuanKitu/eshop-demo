...
export default function CreateProduct() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const activeVariants = variants.filter(v => v.name.trim() !== '' && v.values.some(val => val.trim() !== ''));
      ...
      const variantValues = activeVariants.map(v =>
        Array.from(new Set(v.values.filter(val => val.trim() !== '')))
      );
      ...
      setVariantOptions(prevOptions => {
        return allCombinations.map(combo => {
          ...
          const existing = prevOptions.find(opt => opt.name === name);
          return {
