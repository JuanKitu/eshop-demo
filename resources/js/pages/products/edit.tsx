...
export default function EditProduct() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const activeVariants = variants.filter((v: any) => v.name.trim() !== '' && v.values.some((val: any) => val.trim() !== ''));
      ...
      // Filter empty values and remove duplicates within each variant group
      const variantValues = activeVariants.map((v: any) =>
        Array.from(new Set(v.values.filter((val: any) => val.trim() !== '')))
      );
      const allCombinations = generateCombinations(variantValues);

      setVariantOptions(prevOptions => {
        return allCombinations.map(combo => {
          const name = combo.join(' - ');
          const existing = prevOptions.find((opt: any) => opt.name === name);
          return {
            id: existing ? existing.id : Math.random().toString(36).substring(7),
            name: name,
            price: existing ? existing.price : formData.price.toString(),
            sale_price: existing ? existing.sale_price : formData.sale_price.toString(),
          };
        });
      });
    }, 500);
  ...
  }, [variants, formData.price, formData.sale_price]);
  ...
  return (
    <PageTemplate
      ...
      description={t('Update product details, pricing, and inventory')}
      url="/products/edit"
      actions={pageActions}
      breadcrumbs={[
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                {variants.map((variant: any, index: number) => (
                  <div key={index} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <Input
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                          ...
                          (newVariants[index] as any).name = e.target.value;
                    ...
                    <div className="space-y-2">
                      {(variant as any).values.map((value: string, valueIndex: number) => (
                        <div key={valueIndex} className="flex items-center space-x-2">
                          <Input
                            placeholder={t('Variant value (ex. Small, Red)')}
                            value={value}
                            onChange={(e) => {
                              const newVariants = JSON.parse(JSON.stringify(variants));
                              (newVariants[index] as any).values[valueIndex] = e.target.value;
                              setVariants(newVariants);
                            }}
                          />
                          <Button
                            ...
                            onClick={() => {
                              const newVariants = JSON.parse(JSON.stringify(variants));
                              (newVariants[index] as any).values.splice(valueIndex, 1);
                              if ((newVariants[index] as any).values.length === 0) {
                                (newVariants[index] as any).values.push('');
                              }
                              setVariants(newVariants);
                            }}
                          >
                ...
                {variantOptions.length > 0 && (
                  <div className="mt-6">
                    <h3 className="text-sm font-medium mb-4">{t('Variant Prices')}</h3>
                      ...
                      <table className="w-full text-sm text-left">
                        <thead className="bg-muted">
                          <tr>
                            ...
                            <th className="px-4 py-3 font-medium">{t('Sale Price')}</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y">
                          {variantOptions.map((opt, index) => (
                            <tr key={opt.id} className="bg-card">
                              ...
                              <td className="px-4 py-3">
                                <Input
                                  ...
                                  onChange={(e) => {
                                    const newOptions = [...variantOptions];
                                    newOptions[index].sale_price = e.target.value;
                                    setVariantOptions(newOptions);
                                  }}
                                  placeholder="0.00"
                                  className="w-32"
