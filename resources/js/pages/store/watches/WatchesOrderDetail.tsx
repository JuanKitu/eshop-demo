                    ...
                    <tbody className="bg-white divide-y divide-slate-200">
                      {order.items.map((item) => (
                        <tr key={item.id}>
                          ...
                          <td className="px-6 py-4">
                            <div className="text-sm text-slate-900">{formatCurrency(item.price, storeSettings as any, currencies as any)}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-slate-900">{item.quantity}</div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm font-medium text-slate-900">{formatCurrency(item.price * item.quantity, storeSettings as any, currencies as any)}</div>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="p-6">
                  ...
                  <div className="bg-slate-50 p-4">
                    <div className="flex justify-between py-2 text-slate-600">
                      <span>Subtotal:</span>
                      <span>{formatCurrency(order.subtotal, storeSettings as any, currencies as any)}</span>
                    </div>
                    {order.discount > 0 && (
                      <div className="flex justify-between py-2 text-green-600">
                        <span>Discount:</span>
                        <span>-{formatCurrency(order.discount, storeSettings as any, currencies as any)}</span>
                      </div>
                    )}
                    <div className="flex justify-between py-2 text-slate-600">
                      <span>Shipping:</span>
                      <span>{order.shipping === 0 ? 'Free' : formatCurrency(order.shipping, storeSettings as any, currencies as any)}</span>
                    </div>
                    <div className="flex justify-between py-2 text-slate-600">
                      <span>Tax:</span>
                      <span>{formatCurrency(order.tax, storeSettings as any, currencies as any)}</span>
                    </div>
                    <div className="flex justify-between py-3 border-t border-slate-300 font-medium text-lg text-slate-900">
                      <span>Total:</span>
                      <span>{formatCurrency(order.total, storeSettings as any, currencies as any)}</span>
                    </div>
                  </div>
                </div>
              </div>
