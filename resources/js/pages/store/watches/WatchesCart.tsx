    ...
    return (
      <div className="flex flex-wrap gap-2 mt-2">
        {Object.entries(variantObj).map(([key, value]: [string, any]) => (
          <span key={key} className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800 border border-amber-200 uppercase tracking-tighter">
            {key}: {value}
          </span>
        ))}
      </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                            <div className="space-y-2">
                              ...
                              <div className="flex items-center space-x-3">
                                {item.sale_price ? (
                                  <>
                                    <span className="text-3xl font-light text-slate-900">{formatCurrency(item.sale_price, storeSettings as any, currencies as any)}</span>
                                    <span className="text-lg text-slate-500 line-through">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                    <span className="bg-red-500 text-white px-2 py-1 rounded-md text-xs font-bold">SALE</span>
                                  </>
                                ) : (
                                  <span className="text-3xl font-light text-slate-900">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="space-y-2 text-right">
                              ...
                              <div className="text-3xl font-light text-slate-900">
                                {formatCurrency(((item.sale_price && parseFloat(item.sale_price as any) < parseFloat(item.price as any) ? item.sale_price : item.price)) * item.quantity, storeSettings as any, currencies as any)}
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="space-y-3">
                        ...
                        {dynamicSummary.discount > 0 && (
                          <div className="flex justify-between text-sm text-green-300">
                            <span>Discount</span>
                            <span className="font-medium">-{formatCurrency(dynamicSummary.discount, storeSettings as any, currencies as any)}</span>
                          </div>
                        )}
                        ...
                        <div className="flex justify-between text-sm text-slate-200">
                          ...
                          <span className="font-medium">
                            {dynamicSummary.shipping === 0 ? 'Free' : formatCurrency(dynamicSummary.shipping, storeSettings as any, currencies as any)}
                          </span>
                        </div>
                        ...
                        <div className="flex justify-between text-lg font-medium text-white pt-3 border-t border-slate-600">
                          <span>Total</span>
                          <span>{formatCurrency(dynamicSummary.total, storeSettings as any, currencies as any)}</span>
                        </div>
                      </div>
                    </div>
