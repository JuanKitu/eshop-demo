...
export default function WatchesCheckout({
  ...
  const [whatsappNumber, setWhatsappNumber] = useState(userData.phone || '');
  ...
  const renderVariants = (variants: any) => {
    ...
    if (typeof variants === 'string') {
      try {
        variantObj = JSON.parse(variants);
      } catch (e) {
        return null;
      }
    }

    if (!variantObj || typeof variantObj !== 'object' || Array.isArray(variantObj)) return null;

    return (
      <div className="flex flex-wrap gap-1 mt-1">
        {Object.entries(variantObj).map(([key, value]: [string, any]) => (
          <span key={key} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-amber-100 text-amber-800 border border-amber-200 uppercase tracking-tighter">
            {key}: {value}
          </span>
        ))}
      </div>
  ...
  const handleBillingCountryChange = async (countryId: string) => {
    ...
    if (countryId) {
      ...
      try {
        ...
        console.error('Failed to load billing states:', error);
        setBillingStates([]);
      } finally {
  ...
  const handleBillingStateChange = async (stateId: string) => {
    ...
    if (stateId) {
      ...
      try {
        ...
        console.error('Failed to load billing cities:', error);
        setBillingCities([]);
      } finally {
  ...
  const getShippingCost = () => {
    ...
    const cost = selectedShipping.cost ? parseFloat(String(selectedShipping.cost)) : 0;
    const handlingFee = selectedShipping.handling_fee ? parseFloat(String(selectedShipping.handling_fee)) : 0;
                ...
                {steps.map((step, stepIdx) => {
                  ...
                  return (
                    <div key={step.id} className="flex items-center">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${isCompleted ? 'bg-amber-500 text-slate-900' :
                        isActive ? 'bg-slate-900 text-white' : 'bg-slate-300 text-slate-600'
                        }`}>
                        {isCompleted ? <Check className="h-4 w-4" /> : stepIdx + 1}
                      </div>
                      {stepIdx < steps.length - 1 && (
                        <div className={`flex-1 h-1 mx-4 ${isCompleted ? 'bg-amber-500' : 'bg-slate-300'
                          }`}></div>
                      )}
                    </div>
                  );
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingFirstName ? 'border-red-500' : 'border-slate-300'
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingLastName ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            />
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingEmail ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            />
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingPhone ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            />
                          ...
                          <div className="md:col-span-2">
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingStreet ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            />
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingState ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            >
                          ...
                          <div>
                            ...
                            <select
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingCity ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            >
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full px-3 py-3 border rounded-md ${shippingErrors.shippingZip ? 'border-red-500' : 'border-slate-300'
                                } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                            />
                          ...
                          <div className="space-y-3">
                            {shippingMethods.length > 0 ? (
                              shippingMethods.map((method) => {
                                ...
                                return (
                                  <label key={method.id} className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${selectedShippingId === method.id ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white hover:border-slate-300'
                                    }`}>
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="text-sm font-medium text-slate-900">
                                        {shippingCost === 0 ? 'Free' : formatCurrency(shippingCost, storeSettings as any, currencies as any)}
                                      </div>
                                    </div>
                                  </label>
                                );
                            ...
                            {cartItems.map((item) => {
                              ...
                              return (
                                <div key={item.id} className="flex items-center space-x-4 py-4 border-b border-slate-100">
                                  ...
                                  <div className="text-sm font-medium text-slate-900">
                                    {formatCurrency(itemTotal, storeSettings as any, currencies as any)}
                                  </div>
                                </div>
                              );
                            })}
                          ...
                          {!sameAsShipping && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                              <div>
                                ...
                                <input type="text" value={billingFirstName} onChange={(e) => setBillingFirstName(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none" />
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">Last Name *</label>
                                <input type="text" value={billingLastName} onChange={(e) => setBillingLastName(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none" />
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">Email *</label>
                                <input type="email" value={billingEmail} onChange={(e) => setBillingEmail(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none" />
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">Phone *</label>
                                <input type="tel" value={billingPhone} onChange={(e) => setBillingPhone(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none" />
                              </div>
                              <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-slate-700 mb-2">Street Address *</label>
                                <input type="text" value={billingStreet} onChange={(e) => setBillingStreet(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none" />
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">Country *</label>
                                <select value={billingCountry} onChange={(e) => handleBillingCountryChange(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none">
                                  <option value="">Select Country</option>
                              ...
                              <div>
                                ...
                                <select value={billingState} onChange={(e) => handleBillingStateChange(e.target.value)} disabled={!billingCountry || loadingBillingStates} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none">
                                  <option value="">{loadingBillingStates ? 'Loading...' : 'Select State'}</option>
                                  {billingStates.map((s: any) => <option key={s.id} value={s.id}>{s.name}</option>)}
                                </select>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">City *</label>
                                <select value={billingCity} onChange={(e) => setBillingCity(e.target.value)} disabled={!billingState || loadingBillingCities} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none">
                                  <option value="">{loadingBillingCities ? 'Loading...' : 'Select City'}</option>
                                  {billingCities.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
                                </select>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-slate-700 mb-2">ZIP *</label>
                                <input type="text" value={billingZip} onChange={(e) => setBillingZip(e.target.value)} className="w-full px-3 py-3 border border-slate-300 rounded-md focus:border-amber-500 focus:outline-none" />
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-3">
                            ...
                            {enabledPaymentMethods.cod && (
                              <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${paymentMethod === 'cod' ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white hover:border-slate-300'
                                }`}>
                                <div className="flex items-center">
                                  <input
                                    id="payment-cod"
                            ...
                            {enabledPaymentMethods.whatsapp && (
                              <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${paymentMethod === 'whatsapp' ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white hover:border-slate-300'
                                }`}>
                                <div className="flex items-center">
                                  <input
                                    id="payment-whatsapp"
                            ...
                            {enabledPaymentMethods.telegram && (
                              <label className={`block p-4 border-2 rounded-lg cursor-pointer transition-all ${paymentMethod === 'telegram' ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white hover:border-slate-300'
                                }`}>
                                <div className="flex items-center">
                                  <input
                                    id="payment-telegram"
                            ...
                            {Object.entries(enabledPaymentMethods).filter(([method]) => !['whatsapp', 'telegram', 'cod'].includes(method)).map(([method, config]: [string, any]) => {
                              ...
                              return (
                                <div key={method} className={`border-2 rounded-lg cursor-pointer transition-all ${paymentMethod === method ? 'border-amber-500 bg-amber-50' : 'border-slate-200 bg-white hover:border-slate-300'
                                  }`}>
                          ...
                          <div>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className={`w-full px-3 py-3 border rounded-md ${paymentErrors.whatsappNumber ? 'border-red-500' : 'border-slate-300'
                                  } focus:border-amber-500 focus:ring-1 focus:ring-amber-500 focus:outline-none transition-colors`}
                              />
                  ...
                  <div className="bg-white border border-slate-200 rounded-lg p-6 sticky top-6">
                      ...
                      {cartItems.map((item) => {
                        ...
                        return (
                          <div key={item.id} className="flex items-center space-x-3 p-3 bg-slate-50 rounded-lg">
                            ...
                            <div className="text-sm font-medium text-slate-900">
                              {formatCurrency((parseFloat(String(item.sale_price || item.price)) * item.quantity), storeSettings as any, currencies as any)}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="mb-6 border-t border-slate-200 pt-6">
                      ...
                      <div className="flex">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-r-md text-sm font-medium transition-colors ${couponApplied ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-amber-500 text-slate-900 hover:bg-amber-600'
                            }`}
                        >
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="text-slate-900 font-medium">{formatCurrency(updatedCartSummary.subtotal, storeSettings as any, currencies as any)}</span>
                      </div>
                      {updatedCartSummary.discount > 0 && (
                        <div className="flex justify-between">
                          <span className="text-slate-600">Discount</span>
                          <span className="text-green-600 font-medium">-{formatCurrency(updatedCartSummary.discount, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                        <span className="text-slate-600">Shipping</span>
                        <span className="text-slate-900 font-medium">
                          {updatedCartSummary.shipping === 0 ? 'Free' : formatCurrency(updatedCartSummary.shipping, storeSettings as any, currencies as any)}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-600">Tax</span>
                        <span className="text-slate-900 font-medium">{formatCurrency(updatedCartSummary.tax, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="flex justify-between text-lg font-medium text-slate-900 pt-4 border-t border-slate-100">
                        <span>Total</span>
                        <span className="text-amber-600">{formatCurrency(updatedCartSummary.total, storeSettings as any, currencies as any)}</span>
                      </div>
                    </div>
                  </div>
