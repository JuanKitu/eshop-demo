                        ...
                        <tr key={item.id} className="hover:bg-gray-50">
                              ...
                              <div className="ml-4">
                                ...
                                <div className="md:hidden mt-1">
                                  {item.sale_price && parseFloat(item.sale_price as any) < parseFloat(item.price as any) ? (
                                    <div className="flex items-center">
                                      <span className="text-primary font-bold">{formatCurrency(item.sale_price, storeSettings, currencies)}</span>
                                      <span className="text-gray-500 line-through ml-2 text-xs">{formatCurrency(item.price, storeSettings, currencies)}</span>
                                    </div>
                                  ) : (
                                    <span className="font-bold">{formatCurrency(item.price, storeSettings, currencies)}</span>
                                  )}
                                </div>
                          ...
                          <td className="py-4 px-6 hidden md:table-cell">
                            {item.sale_price && parseFloat(item.sale_price as any) < parseFloat(item.price as any) ? (
                              <div className="flex items-center">
                                <span className="text-primary font-bold">{formatCurrency(item.sale_price, storeSettings, currencies)}</span>
                                <span className="text-gray-500 line-through ml-2 text-sm">{formatCurrency(item.price, storeSettings, currencies)}</span>
                              </div>
                            ) : (
                              <span className="font-bold">{formatCurrency(item.price, storeSettings, currencies)}</span>
                            )}
