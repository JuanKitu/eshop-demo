                  ...
                  {order.items.map((item) => (
                    <div key={item.id} className="flex items-center space-x-4 p-4 bg-stone-50 rounded-xl border border-purple-100">
                      ...
                      <div className="flex-1">
                        <h4 className="text-lg font-medium text-gray-900">{item.name || (item as any).product?.name}</h4>
                        {(item as any).variant_name && (item as any).variant_name !== 'default' && (
                          <p className="text-sm text-purple-600 mt-1">{(item as any).variant_name}</p>
                        )}
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-lg font-medium text-purple-800">{formatCurrency(parseFloat(String(item.price)) as any, storeSettings as any, currencies as any)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm text-gray-600">Total</p>
                        <p className="text-lg font-medium text-purple-800">{formatCurrency((parseFloat(String(item.price)) * item.quantity) as any, storeSettings as any, currencies as any)}</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {order.subtotal !== undefined && (
                    <div className="flex justify-between text-purple-200">
                      <span>Subtotal</span>
                      <span className="text-white">{formatCurrency(parseFloat(String(order.subtotal)) as any, storeSettings as any, currencies as any)}</span>
                    </div>
                  )}
                  {order.discount !== undefined && order.discount > 0 && (
                    <div className="flex justify-between text-green-400">
                      <span>Discount</span>
                      <span>-{formatCurrency(parseFloat(String(order.discount)) as any, storeSettings as any, currencies as any)}</span>
                    </div>
                  )}
                  {order.shipping !== undefined && (
                    <div className="flex justify-between text-purple-200">
                      <span>Shipping</span>
                      <span className="text-white">{order.shipping === 0 ? 'Free' : formatCurrency(parseFloat(String(order.shipping)) as any, storeSettings as any, currencies as any)}</span>
                    </div>
                  )}
                  {order.tax !== undefined && (
                    <div className="flex justify-between text-purple-200">
                      <span>Tax</span>
                      <span className="text-white">{formatCurrency(parseFloat(String(order.tax)) as any, storeSettings as any, currencies as any)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-xl font-medium text-white pt-3 border-t-2 border-amber-400">
                    <span>Total</span>
                    <span className="text-amber-400">{formatCurrency(parseFloat(String(order.total)) as any, storeSettings as any, currencies as any)}</span>
                  </div>
                </div>
              </div>
            </div>
