                            ...
                            <div className="flex justify-between items-start mb-2">
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-1">
                                  ...
                                  {item.sale_price && parseFloat(String(item.sale_price)) > 0 && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)) && (
                                    <span className="bg-red-100 text-red-700 px-1.5 py-0.5 text-xs font-medium rounded">
                                      -{Math.round(((parseFloat(String(item.price)) - parseFloat(String(item.sale_price))) / parseFloat(String(item.price))) * 100)}%
                                    </span>
                                  )}
                                </div>
                                ...
                                <div className="space-y-1">
                                  {item.category && (
                                    <p className="text-sm text-purple-600 font-medium">{item.category.name}</p>
                                  )}

                                  {/* Variants - Moved below category */}
                                  {(item.variants || item.selected_variants) && Object.keys(item.variants || item.selected_variants).length > 0 && (
                                    <div className="flex flex-wrap gap-2 my-1.5">
                                      {Object.entries(item.variants || item.selected_variants).map(([key, value]) => (
                                        <span key={key} className="bg-purple-50 text-purple-700 px-2 py-0.5 text-[10px] uppercase tracking-wider rounded border border-purple-100 font-medium">
                                          {key}: {String(value)}
                                        </span>
                                      ))}
                                    </div>
                                  ...
                                  {/* Stock Status */}
                                  <div className={`inline-flex items-center gap-1 text-xs ${item.stock > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                    <div className={`w-1.5 h-1.5 rounded-full ${item.stock > 0 ? 'bg-green-500' : 'bg-red-500'}`}></div>
                                ...
                                {item.sale_price && parseFloat(String(item.sale_price)) > 0 && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)) ? (
                                  <>
                                    <span className="text-lg font-semibold text-purple-800">
                                      {formatCurrency(parseFloat(String(item.sale_price)) as any, storeSettings as any, currencies as any)}
                                    </span>
                                    <span className="text-sm text-gray-400 line-through">
                                      {formatCurrency(parseFloat(String(item.price)) as any, storeSettings as any, currencies as any)}
                                    </span>
                                  </>
                                ) : (
                                  <span className="text-lg font-semibold text-purple-800">
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="flex justify-between items-center">
                                ...
                                <span className="font-semibold text-purple-800">
                                  {(() => {
                                    const unitPrice = (item.sale_price && parseFloat(String(item.sale_price)) > 0 && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)))
                                      ? parseFloat(String(item.sale_price))
                                      : parseFloat(String(item.price));
                                    return formatCurrency((unitPrice * item.quantity) as any, storeSettings as any, currencies as any);
                                  })()}
                                </span>
                              </div>
                            </div>
                  ...
                  <div className="bg-stone-50 rounded-2xl p-6 sticky top-8">
                    ...
                    <div className="space-y-4 mb-6">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatCurrency(summary.subtotal as any, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Shipping</span>
                        <span className="font-medium">{summary.shipping === 0 ? 'Free' : formatCurrency(summary.shipping as any, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Tax</span>
                        <span className="font-medium">{formatCurrency(summary.tax || 0 as any, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="border-t border-gray-200 pt-4">
                        <div className="flex justify-between">
                          <span className="text-lg font-medium text-gray-900">Total</span>
                          <span className="text-lg font-semibold text-purple-800">{formatCurrency(summary.total as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      </div>
                    </div>
...
export default function PerfumeCart(props: PerfumeCartProps) {
  return (
    <>
      ...
      <CartProvider storeId={props.store.id} isLoggedIn={props.isLoggedIn || false}>
        <WishlistProvider>
