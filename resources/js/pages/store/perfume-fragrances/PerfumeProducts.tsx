...
function WishlistButton({ productId }: { productId: number }) {
  ...
  return (
    <button
      ...
      className={`w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-md hover:bg-white hover:scale-110 transition-all duration-300 ${isInWishlist(productId) ? 'text-red-500' : 'text-gray-600'
        }`}
    >
...
export default function PerfumeProducts({
  ...
  const storeSlug = props.store?.slug || (store as any).slug || 'perfume-store';
  ...
  const getProductPrice = (product: any) => {
    let minPrice: number | null = parseFloat(product.price);
    let maxPrice: number | null = parseFloat(product.price);
    let minSalePrice: number | null = product.sale_price ? parseFloat(product.sale_price) : null;
    let maxSalePrice: number | null = product.sale_price ? parseFloat(product.sale_price) : null;
    ...
    if (product.variants) {
      try {
        ...
        if (variants && typeof variants === 'object') {
          hasVariants = Array.isArray(variants) ? variants.length > 0 : Object.keys(variants).length > 0;

          if (!Array.isArray(variants) && Object.keys(variants).length > 0) {
            const prices = Object.values(variants).map((v: any) => parseFloat(v.price)).filter((p: number) => !isNaN(p));
            if (prices.length > 0) {
                ...
                {products.length > 0 ? (
                  <div className={viewMode === 'grid'
                    ...
                    {products.map((product) => {
                      const { minPrice, maxPrice, minSalePrice, maxSalePrice, hasVariants } = getProductPrice(product);
                          ...
                          if (minPrice !== null && maxPrice !== null) {
                            return (
                              <span className="text-xl font-semibold text-purple-800">
                                {formatCurrency(minPrice as any, storeSettings as any, currencies as any)} - {formatCurrency(maxPrice as any, storeSettings as any, currencies as any)}
                              </span>
                            );
                          }
                        }

                        if (minSalePrice && minSalePrice < (minPrice || 0)) {
                          return (
                            <>
                              <span className="text-xl font-semibold text-purple-800">
                                {formatCurrency(minSalePrice as any, storeSettings as any, currencies as any)}
                              </span>
                              <span className="text-gray-500 line-through text-sm">
                                {formatCurrency((minPrice || 0) as any, storeSettings as any, currencies as any)}
                              </span>
                            </>
                          );
                        }
                        ...
                        <div key={product.id} className="bg-white rounded-2xl shadow-md border border-purple-100 hover:shadow-lg transition-shadow duration-300 overflow-hidden">
                          <div className="flex h-40">
                            <Link href={generateStoreUrl('store.product', store, { id: product.id })} className="w-32 flex-shrink-0 relative overflow-hidden rounded-l-2xl bg-purple-50">
                              ...
                              {hasVariants && (
                                <span className="absolute top-10 right-2 bg-purple-800 text-white px-2 py-1 text-[10px] font-medium rounded-full">
                                  Variants
                                </span>
                              )}
                            </Link>
                                  ...
                                  <div className="relative">
                                    {hasVariants ? (
                                      <Link
                                        ...
                                        className="w-10 h-10 bg-purple-100 hover:bg-purple-200 text-purple-800 rounded-full flex items-center justify-center transition-colors duration-300 shadow-md"
                                      >
                                        ...
                                      </Link>
                                    ) : (
                      ...
                      {Array.from({ length: Math.min(5, pagination.last_page) }, (_, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-4 py-2 border rounded-full text-sm ${page === pagination.current_page
