...
export default function PerfumeOrderConfirmation({
  ...
  const storeSlug = props.store?.slug || (store as any).slug || 'demo';
                      ...
                      return (
                        <div key={item.id} className="flex items-center space-x-6 p-6 bg-stone-50 rounded-xl border border-purple-100">
                          ...
                          <div className="flex-1">
                            <div className="text-xl font-medium text-gray-900">{item.name || (item as any).product?.name}</div>
                            {(item as any).variant_name && (item as any).variant_name !== 'default' && (
                              <div className="text-sm text-purple-600 mt-1">{(item as any).variant_name}</div>
                            )}
                    ...
                    <div className="space-y-4">
                      {orderData.subtotal && (
                        <div className="flex justify-between text-purple-200 border-b border-purple-700 pb-2">
                          ...
                          <span className="font-medium text-white">{formatCurrency(parseFloat(String(orderData.subtotal)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      {orderData.discount && orderData.discount > 0 && (
                        <div className="flex justify-between text-green-400 border-b border-purple-700 pb-2">
                          ...
                          <span className="font-medium">-{formatCurrency(parseFloat(String(orderData.discount)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      {orderData.shipping && orderData.shipping > 0 && (
                        <div className="flex justify-between text-purple-200 border-b border-purple-700 pb-2">
                          <span className="font-medium">Shipping</span>
                          <span className="font-medium text-white">{formatCurrency(parseFloat(String(orderData.shipping)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      {orderData.tax && orderData.tax > 0 && (
                        <div className="flex justify-between text-purple-200 border-b border-purple-700 pb-2">
                          <span className="font-medium">Tax</span>
                          <span className="font-medium text-white">{formatCurrency(parseFloat(String(orderData.tax)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      <div className="flex justify-between text-2xl font-medium text-white pt-4 border-t-2 border-amber-400">
                        <span>Total</span>
                        <span className="text-amber-400">{formatCurrency(parseFloat(String(orderData.total)) as any, storeSettings as any, currencies as any)}</span>
                      </div>
                    </div>
                  </div>
                </div>
