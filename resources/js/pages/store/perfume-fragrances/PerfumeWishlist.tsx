  ...
  function WishlistInner() {
    ...
    const handleAddToCart = async (item: any) => {
      ...
      if (variants && variants.length > 0) {
        // Redirect to detail page if selection is needed
    ...
    return (
      <>
                  ...
                  {items.map((item) => (
                    <div key={item.id} className="group relative bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300">
                      ...
                      <div className="relative aspect-square overflow-hidden">
                        <img
                          src={getImageUrl(item.cover_image) || `https://placehold.co/250x250/fafaf9/7c3aed?text=${encodeURIComponent(item.name)}`}
                          alt={item.name}
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mb-3">
                          {item.sale_price && parseFloat(String(item.sale_price)) > 0 && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)) ? (
                            <div className="flex items-center space-x-1">
                              <span className="text-purple-800 font-semibold text-sm">{formatCurrency(parseFloat(String(item.sale_price)) as any, storeSettings as any, currencies as any)}</span>
                              <span className="text-gray-400 line-through text-xs">{formatCurrency(parseFloat(String(item.price)) as any, storeSettings as any, currencies as any)}</span>
                            </div>
                          ) : (
                            <span className="text-purple-800 font-semibold text-sm">{formatCurrency(parseFloat(String(item.price)) as any, storeSettings as any, currencies as any)}</span>
                          )}
                        </div>
                        ...
                        <div className="flex space-x-2">
                          ...
                          <button
                            ...
                            className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors duration-300 ${item.stock <= 0
        ...
        {items.length === 0 && relatedProducts && (relatedProducts as any[]).length > 0 && (
          <section className="py-16 bg-stone-50">
            <div className="container mx-auto px-4">
