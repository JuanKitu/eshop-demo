    ...
    return (
      <div className="flex flex-wrap gap-2 mb-2">
        {Object.entries(parsedVariants).map(([key, value]) => (
          <span key={key} className="text-[10px] text-yellow-700 italic bg-yellow-50 px-2 py-0.5 rounded-full border border-yellow-200">
            {key}: {String(value)}
          </span>
        ))}
      </div>
                        ...
                        <div className="flex items-start space-x-6">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="flex items-center space-x-3 mb-4">
                              {item.sale_price ? (
                                <>
                                  <span className="text-2xl font-bold text-gray-800">{formatCurrency(item.sale_price, storeSettings as any, currencies as any)}</span>
                                  <span className="text-lg text-gray-500 line-through">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                  <span className="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs font-medium">SALE</span>
                                </>
                              ) : (
                                <span className="text-2xl font-bold text-gray-800">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                              )}
                            </div>
                      ...
                      <div className="bg-white/10 rounded-xl p-4 mb-6">
                        <div className="space-y-3">
                          <div className="flex justify-between text-sm text-yellow-100">
                            <span className="font-semibold">{formatCurrency(dynamicSummary.subtotal, storeSettings as any, currencies as any)}</span>
                          </div>
                          ...
                          <div className="flex justify-between text-sm text-yellow-100">
                            <span>Tax</span>
                            <span className="font-semibold">{formatCurrency(dynamicSummary.tax, storeSettings as any, currencies as any)}</span>
                          </div>
