    ...
    return (
      <div className="flex flex-wrap gap-2 mt-1 mb-2">
        {Object.entries(parsedVariants).map(([key, value]) => (
          <span key={key} className="text-xs text-yellow-700 italic bg-yellow-50 px-2.5 py-1 rounded-full border border-yellow-100 shadow-sm">
            {key}: {String(value)}
          </span>
        ))}
      </div>
                            ...
                            <div className="flex items-center space-x-6 p-6 bg-gradient-to-r from-yellow-50/70 to-amber-50/70 rounded-2xl border border-yellow-200/60 hover:shadow-lg hover:border-yellow-300 transition-all duration-300">
                              ...
                              <div className="flex-1">
                                ...
                                <div className="flex items-center space-x-4 text-sm">
                                  ...
                                  <div className="text-gray-600">
                                    <span className="font-medium">{formatCurrency(item.price || 0, storeSettings as any, currencies as any)}</span> each
                                  </div>
                                </div>
                              ...
                              <div className="text-right">
                                <div className="text-2xl font-bold text-gray-800 mb-1">
                                  {formatCurrency((parseFloat(String(item.price || 0)) * item.quantity), storeSettings as any, currencies as any)}
                                </div>
                      ...
                      <div className="bg-white/10 rounded-xl p-4 mb-6">
                        <div className="space-y-3">
                          <div className="flex justify-between text-sm text-yellow-100">
                            ...
                            <span className="font-semibold">{formatCurrency(order.subtotal || 0, storeSettings as any, currencies as any)}</span>
                          </div>

                          {(parseFloat(String(order.discount || 0))) > 0 && (
                            <div className="flex justify-between text-sm text-green-200">
                              <span>Special Discount</span>
                              <span className="font-semibold">-{formatCurrency(order.discount || 0, storeSettings as any, currencies as any)}</span>
                            </div>
                          )}

                          <div className="flex justify-between text-sm text-yellow-100">
                            <span>Shipping</span>
                            <span className="font-semibold">{formatCurrency(order.shipping || 0, storeSettings as any, currencies as any)}</span>
                          </div>

                          <div className="flex justify-between text-sm text-yellow-100">
                            <span>Tax</span>
                            <span className="font-semibold">{formatCurrency(order.tax || 0, storeSettings as any, currencies as any)}</span>
                          </div>

                          <div className="flex justify-between text-lg font-bold text-white pt-3 border-t border-yellow-400">
                            <span>Total Paid</span>
                            <span>{formatCurrency(order.total || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        </div>
                      </div>
