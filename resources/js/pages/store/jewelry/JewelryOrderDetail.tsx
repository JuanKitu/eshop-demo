                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {(order.items || []).map((item) => (
                          <tr key={item.id}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{formatCurrency(item.price || 0, storeSettings as any, currencies as any)}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{item.quantity}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{formatCurrency((parseFloat(String(item.price || 0)) * item.quantity), storeSettings as any, currencies as any)}</div>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                <div className="p-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-md">
                    <div className="flex justify-between py-2">
                      <span>Subtotal:</span>
                      <span>{formatCurrency(order.subtotal || 0, storeSettings as any, currencies as any)}</span>
                    </div>
                    {(parseFloat(String(order.discount || 0))) > 0 && (
                      <div className="flex justify-between py-2 text-green-600">
                        <span>Discount:</span>
                        <span>-{formatCurrency(order.discount || 0, storeSettings as any, currencies as any)}</span>
                      </div>
                    )}
                    <div className="flex justify-between py-2">
                      <span>Shipping:</span>
                      <span>{formatCurrency(order.shipping || 0, storeSettings as any, currencies as any)}</span>
                    </div>
                    <div className="flex justify-between py-2">
                      <span>Tax:</span>
                      <span>{formatCurrency(order.tax || 0, storeSettings as any, currencies as any)}</span>
                    </div>
