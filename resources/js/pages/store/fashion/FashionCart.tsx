                        ...
                        {item.variants && Object.entries(item.variants).length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-2">
                            {Object.entries(item.variants).map(([key, value]) => (
                              <span key={key} className="text-xs text-gray-500 italic">
                                {key}: {String(value)}
                              </span>
                            ))}
                          </div>
                          ...
                          <div className="flex items-center space-x-2 mb-4">
                            {item.sale_price ? (
                            <>
                              <span className="text-lg font-medium">{formatCurrency(item.sale_price, storeSettings as any, currencies as any)}</span>
                              <span className="text-sm text-gray-500 line-through">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                            </>
                          ) : (
                            <span className="text-lg font-medium">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                          )}
                          </div>
                    ...
                    <div className="space-y-4 mb-6">
                      ...
                      <div className="border-t pt-4">
                        <div className="flex justify-between text-lg">
                          ...
                          <span className="font-medium">{formatCurrency(dynamicSummary.total, storeSettings as any, currencies as any)}</span>
                        </div>
                      </div>
                    </div>
