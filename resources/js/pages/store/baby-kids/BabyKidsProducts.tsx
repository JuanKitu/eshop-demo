...
function WishlistButton({ productId }: { productId: number }) {
  ...
  return (
    <button
      ...
      className={`p-3 rounded-full transition-all duration-300 shadow-lg transform hover:scale-110 ${isInWishlist(productId)
...
export default function BabyKidsProducts({
  ...
  const { props } = usePage();
  const storeSlug = (props as any).store?.slug || 'baby-kids-store';
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="flex items-center justify-between">
                                      <div className="flex items-center gap-2">
                                        {product.sale_price ? (
                                          <>
                                            <span className="text-lg font-bold text-red-500">{formatCurrency(product.sale_price as any, storeSettings as any, currencies as any)}</span>
                                            <span className="text-sm text-gray-500 line-through">{formatCurrency(product.price as any, storeSettings as any, currencies as any)}</span>
                                          </>
                                        ) : (
                                          <span className="text-lg font-bold text-gray-800">{formatCurrency(product.price as any, storeSettings as any, currencies as any)}</span>
                                        )}
                                      </div>
                                      <div className="flex items-center">
                      ...
                      {Array.from({ length: Math.min(5, pagination.last_page) }, (_, i) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-4 py-2 border-2 rounded-xl text-sm ${page === pagination.current_page
