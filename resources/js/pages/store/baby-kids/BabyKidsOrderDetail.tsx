                      ...
                      <tbody>
                        {order.items?.map((item: any) => (
                          <tr key={item.id} className="border-b border-gray-100">
                              ...
                              <div className="flex items-center space-x-3">
                                ...
                                <div>
                                  ...
                                  {item.variant_name && item.variant_name !== 'default' && (
                                    <p className="text-xs text-pink-600 font-medium mt-1">{item.variant_name}</p>
                                  )}
                                  {item.variants && Object.entries(item.variants).length > 0 && (
                                    <div className="text-xs text-pink-500 font-medium mt-1">
                                      {Object.entries(item.variants).map(([key, value]) => (
                                        <span key={key} className="mr-2">
                                          {key}: <span className="text-pink-700">{String(value)}</span>
                                        </span>
                                      ))}
                                    </div>
                                  )}
                            ...
                            <td className="py-4 px-2 text-right">{formatCurrency(parseFloat(String(item.price || 0)) as any, storeSettings as any, currencies as any)}</td>
                            <td className="py-4 px-2 text-right font-semibold">{formatCurrency((parseFloat(String(item.price || 0)) * parseInt(String(item.quantity || 1))) as any, storeSettings as any, currencies as any)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ...
                  <div className="max-w-sm ml-auto">
                    <div className="space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span>{formatCurrency(parseFloat(String(order.subtotal || 0)) as any, storeSettings as any, currencies as any)}</span>
                      </div>
                      {order.discount > 0 && (
                        <div className="flex justify-between text-green-600">
                          <span>Discount:</span>
                          <span>-{formatCurrency(parseFloat(String(order.discount || 0)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                        <span className="text-gray-600">Shipping:</span>
                        <span>{formatCurrency(parseFloat(String(order.shipping || 0)) as any, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Tax:</span>
                        <span>{formatCurrency(parseFloat(String(order.tax || 0)) as any, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="border-t-2 pt-2">
                        <div className="flex justify-between text-xl font-bold">
                          <span>Total:</span>
                          <span>{formatCurrency(parseFloat(String(order.total || 0)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
