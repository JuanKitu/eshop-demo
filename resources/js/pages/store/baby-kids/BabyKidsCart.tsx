...
function BabyKidsCartContent({
  ...
  const { props } = usePage();
  const storeSlug = (props.store as any)?.slug || props.theme || 'baby-kids';
                        ...
                        <div className="relative bg-white rounded-3xl shadow-xl border-4 border-pink-400 overflow-hidden hover:shadow-2xl transition-all duration-300">
                          <div className="flex flex-col md:flex-row h-full">
                            ...
                            <div className="relative w-full md:w-40 h-48 md:h-auto overflow-hidden flex-shrink-0">
                              <a href={generateStoreUrl('store.product', store, { id: item.product_id })}>
                                <img
                              ...
                              {item.variants && Object.entries(item.variants).length > 0 && (
                                <div className="flex flex-wrap gap-2 mb-3">
                                  {Object.entries(item.variants).map(([key, value]) => (
                                    <span key={key} className="text-xs text-pink-600 bg-pink-100 px-2 py-1 rounded-full font-semibold">
                                      {key}: {String(value)}
                                    </span>
                                  ))}
                                </div>
                                  ...
                                  {item.sale_price && parseFloat(item.sale_price as any) < parseFloat(item.price as any) ? (
                                    <>
                                      <span className="text-lg font-bold text-red-500">
                                        {formatCurrency(item.sale_price as any, storeSettings as any, currencies as any)}
                                      </span>
                                      <span className="text-sm text-gray-500 line-through">
                                        {formatCurrency(item.price as any, storeSettings as any, currencies as any)}
                                      </span>
                                    </>
                                  ) : (
                              ...
                              <div className="mt-4 pt-4 border-t border-pink-200">
                                <div className="flex justify-between items-center">
                                  ...
                                  <span className="text-xl font-bold text-gray-800">
                                    {formatCurrency(itemSubtotal as any, storeSettings as any, currencies as any)}
                                  </span>
                                </div>
                              </div>
                            </div>
                ...
                <div className="relative bg-white rounded-3xl shadow-xl border-4 border-blue-400 p-8 sticky top-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between">
                      <span className="text-gray-600">Subtotal</span>
                      <span className="font-bold">{formatCurrency(dynamicSummary.subtotal as any, storeSettings as any, currencies as any)}</span>
                    </div>

                    {dynamicSummary.discount > 0 && (
                      <div className="flex justify-between text-green-600">
                        <span>Discount</span>
                        <span>-{formatCurrency(dynamicSummary.discount as any, storeSettings as any, currencies as any)}</span>
                      </div>
                    )}

                    <div className="flex justify-between">
                      <span className="text-gray-600">Shipping</span>
                      <span className="font-bold">{formatCurrency(dynamicSummary.shipping as any, storeSettings as any, currencies as any)}</span>
                    </div>

                    <div className="flex justify-between">
                      <span className="text-gray-600">Tax</span>
                      <span className="font-bold">{formatCurrency(dynamicSummary.tax as any, storeSettings as any, currencies as any)}</span>
                    </div>

                    <div className="border-t border-pink-200 pt-4 mt-4">
                      <div className="flex justify-between">
                        <span className="text-xl font-bold">Total</span>
                        <span className="text-xl font-bold text-pink-500">{formatCurrency(dynamicSummary.total as any, storeSettings as any, currencies as any)}</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-6">
                      <form onSubmit={handleApplyCoupon} className="space-y-3">
                        ...
                        <button
                          type="submit"
                          className={`w-full py-3 rounded-2xl font-bold transition-colors ${couponApplied
