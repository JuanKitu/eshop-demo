  ...
  const renderVariants = (variants: any) => {
    ...
    if (typeof variants === 'string') {
      try {
        ...
        return <span className="text-xs text-gray-500">{variants}</span>;
      }
    }

    if (typeof variantObj !== 'object' || variantObj === null) {
      return <span className="text-xs text-gray-500">{String(variantObj)}</span>;
    }
                        ...
                        <tbody className="bg-white">
                          {order.items.map((item, index) => (
                            <tr key={item.id} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                              <td className="p-4">
                                <div className="flex items-center space-x-4">
                                  ...
                                  <div>
                                    ...
                                    {renderVariants(item.variants || item.variant)}
                              ...
                              <td className="p-4 text-right font-bold text-gray-900 text-lg">{formatCurrency(parseFloat(String(item.price)) as any, storeSettings as any, currencies as any)}</td>
                              <td className="p-4 text-right font-bold text-red-600 text-lg">{formatCurrency((parseFloat(String(item.price)) * item.quantity) as any, storeSettings as any, currencies as any)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      <div className="bg-black text-white p-8">
                        ...
                        <div className="space-y-4">
                          <div className="flex justify-between py-2 border-b border-gray-700">
                            ...
                            <span className="font-black text-white">{formatCurrency(parseFloat(String(order.subtotal)) as any, storeSettings as any, currencies as any)}</span>
                          </div>
                          {parseFloat(String(order.discount)) > 0 && (
                            <div className="flex justify-between py-2 border-b border-gray-700">
                              <span className="text-green-400 font-bold tracking-wider uppercase">Discount:</span>
                              <span className="font-black text-green-400">-{formatCurrency(parseFloat(String(order.discount)) as any, storeSettings as any, currencies as any)}</span>
                            </div>
                          )}
                          <div className="flex justify-between py-2 border-b border-gray-700">
                            <span className="text-gray-300 font-bold tracking-wider uppercase">Shipping:</span>
                            <span className="font-black text-white">{formatCurrency(parseFloat(String(order.shipping)) as any, storeSettings as any, currencies as any)}</span>
                          </div>
                          <div className="flex justify-between py-2 border-b border-gray-700">
                            <span className="text-gray-300 font-bold tracking-wider uppercase">Tax:</span>
                            <span className="font-black text-white">{formatCurrency(parseFloat(String(order.tax)) as any, storeSettings as any, currencies as any)}</span>
                          </div>
                          <div className="border-t-2 border-red-600 pt-4">
                            <div className="flex justify-between">
                              ...
                              <span className="text-2xl font-black text-red-400">{formatCurrency(parseFloat(String(order.total)) as any, storeSettings as any, currencies as any)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
