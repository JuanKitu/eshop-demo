...
export default function CarsOrderConfirmation({
  ...
  const { props } = usePage();
  const storeSlug = (props.store as any)?.slug || (store as any).slug || 'demo';
                      ...
                      return (
                        <div key={item.id} className="flex items-center space-x-6 p-6 bg-gray-50 border-2 border-gray-200">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="text-sm text-gray-600">Unit Price: {formatCurrency(parseFloat(String(item.price)) as any, storeSettings as any, currencies as any)}</div>
                          </div>
                          <div className="text-2xl font-bold text-red-600">
                            {formatCurrency(parseFloat(String(itemTotal)) as any, storeSettings as any, currencies as any)}
                          </div>
                        </div>
                      );
                    })}
                    ...
                    <div className="space-y-4">
                      {orderData.subtotal && (
                        <div className="flex justify-between text-gray-300 border-b border-gray-700 pb-2">
                          ...
                          <span className="font-black text-white">{formatCurrency(parseFloat(String(orderData.subtotal)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      {orderData.discount && orderData.discount > 0 && (
                        <div className="flex justify-between text-green-400 border-b border-gray-700 pb-2">
                          ...
                          <span className="font-black">-{formatCurrency(parseFloat(String(orderData.discount)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      {orderData.shipping && orderData.shipping > 0 && (
                        <div className="flex justify-between text-gray-300 border-b border-gray-700 pb-2">
                          <span className="font-bold tracking-wider uppercase">Shipping</span>
                          <span className="font-black text-white">{formatCurrency(parseFloat(String(orderData.shipping)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      {orderData.tax && orderData.tax > 0 && (
                        <div className="flex justify-between text-gray-300 border-b border-gray-700 pb-2">
                          <span className="font-bold tracking-wider uppercase">Tax</span>
                          <span className="font-black text-white">{formatCurrency(parseFloat(String(orderData.tax)) as any, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}
                      <div className="flex justify-between text-2xl font-black text-white pt-4 border-t-2 border-red-600">
                        ...
                        <span className="text-red-400">{formatCurrency(parseFloat(String(orderData.total)) as any, storeSettings as any, currencies as any)}</span>
                      </div>
                    </div>
                  </div>
                </div>
