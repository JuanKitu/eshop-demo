...
export default function FurnitureOrderConfirmation({
  ...
  const { props } = usePage();
  const storeSlug = (props.store as any)?.slug || store.slug || 'demo';
  ...
  const renderVariants = (item: any) => {
    ...
    if (!variants || typeof variants !== 'object' || Object.keys(variants).length === 0) {
      return item.variant_name ? (
        <div className="text-sm text-amber-800 bg-amber-100 px-2 py-0.5 rounded inline-block mt-1">
          {item.variant_name}
        </div>
      ) : null;
    }
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-1">
        {Object.entries(variants).map(([key, value]: [string, any]) => (
          <span key={key} className="text-xs text-amber-800 bg-amber-100 px-2 py-0.5 rounded font-medium">
            {key}: {value}
          </span>
        ))}
      </div>
                  ...
                  <div className="mb-8">
                        ...
                        return (
                          <div key={item.id} className="flex items-center space-x-4 p-6 bg-amber-50 rounded-2xl border border-amber-200">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-sm text-slate-600">Price: {formatCurrency(item.price as any, storeSettings, currencies)}</div>
                            </div>
                            <div className="text-xl font-bold text-amber-800">
                              {formatCurrency(itemTotal as any, storeSettings, currencies)}
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="mt-8 bg-amber-50 rounded-2xl p-6 border border-amber-200">
                      <div className="space-y-3">
                        {orderData.subtotal && parseFloat(String(orderData.subtotal)) > 0 && (
                          <div className="flex justify-between text-slate-700">
                            <span>Subtotal</span>
                            <span>{formatCurrency(orderData.subtotal as any, storeSettings, currencies)}</span>
                          </div>
                        )}
                        {orderData.discount && parseFloat(String(orderData.discount)) > 0 && (
                          <div className="flex justify-between text-green-600">
                            <span>Discount {orderData.coupon_code && `(${orderData.coupon_code})`}</span>
                            <span>-{formatCurrency(orderData.discount as any, storeSettings, currencies)}</span>
                          </div>
                        )}
                        {orderData.shipping && parseFloat(String(orderData.shipping)) > 0 && (
                          <div className="flex justify-between text-slate-700">
                            <span>Delivery</span>
                            <span>{formatCurrency(orderData.shipping as any, storeSettings, currencies)}</span>
                          </div>
                        )}
                        {orderData.tax && parseFloat(String(orderData.tax)) > 0 && (
                          <div className="flex justify-between text-slate-700">
                            <span>Tax</span>
                            <span>{formatCurrency(orderData.tax as any, storeSettings, currencies)}</span>
                          </div>
                        )}
                        <div className="flex justify-between text-xl font-bold text-amber-800 pt-3 border-t-2 border-amber-300">
                          <span>Total</span>
                          <span>{formatCurrency(orderData.total as any, storeSettings, currencies)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
