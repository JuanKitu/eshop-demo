                  ...
                  <div key={item.id} className="group bg-white rounded-2xl shadow-lg shadow-amber-200/20 hover:shadow-2xl hover:shadow-amber-300/30 transition-all duration-500 overflow-hidden border-2 border-amber-100 hover:border-amber-300">
                    <div className="flex flex-col lg:flex-row">
                      <div className="relative w-32 lg:w-40 aspect-square overflow-hidden bg-gradient-to-br from-amber-50 to-yellow-50">
                        ...
                        {(() => {
                          const hasDiscount = item.sale_price && parseFloat(String(item.sale_price)) > 0 && parseFloat(String(item.sale_price)) < parseFloat(String(item.price));
                          return hasDiscount ? (
                            <div className="absolute top-2 left-2 bg-red-500 text-white px-3 py-2 text-xs font-bold rounded-xl shadow-lg">
                              ...
                              <span className="block text-sm leading-none">{Math.round(((parseFloat(String(item.price)) - parseFloat(String(item.sale_price))) / parseFloat(String(item.price))) * 100)}%</span>
                            </div>
                          ) : null;
                        })()}
                      </div>
                                ...
                                return (
                                  <>
                                    <span className="text-lg font-bold text-amber-800">
                                      {formatCurrency(parseFloat(String(item.sale_price)) as any, storeSettings, currencies)}
                                    </span>
                                    <span className="text-base text-slate-400 line-through">
                                      {formatCurrency(parseFloat(String(item.price)) as any, storeSettings, currencies)}
                                    </span>
                                    <span className="bg-red-100 text-red-700 text-sm font-bold px-2 py-1 rounded-lg">
                                      Save {formatCurrency((parseFloat(String(item.price)) - parseFloat(String(item.sale_price))) as any, storeSettings, currencies)}
                                    </span>
                                  </>
                                );
                              } else {
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div className="text-right">
                              ...
                              <div className="text-xl font-bold text-amber-800">
                                {formatCurrency((parseFloat(String(item.sale_price && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)) ? item.sale_price : item.price)) * item.quantity) as any, storeSettings, currencies)}
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                <div className="bg-amber-50 rounded-2xl p-6 shadow-lg border-2 border-amber-100 sticky top-6">
                  ...
                  <div className="space-y-4 mb-6">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-slate-900">{formatCurrency(parseFloat(String(summary.subtotal)) as any, storeSettings, currencies)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Shipping</span>
                      <span className="font-bold text-slate-900">
                        {parseFloat(String(summary.shipping)) === 0 ? 'Free' : formatCurrency(parseFloat(String(summary.shipping)) as any, storeSettings, currencies)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-600">Tax</span>
                      <span className="font-bold text-slate-900">{formatCurrency(parseFloat(String(summary.tax)) as any, storeSettings, currencies)}</span>
                    </div>
                    <div className="border-t-2 border-amber-200 pt-4">
                      <div className="flex justify-between text-xl font-bold">
                        <span className="text-slate-900">Total</span>
                        <span className="text-amber-800">{formatCurrency(parseFloat(String(summary.total)) as any, storeSettings, currencies)}</span>
                      </div>
                    </div>
                  </div>
