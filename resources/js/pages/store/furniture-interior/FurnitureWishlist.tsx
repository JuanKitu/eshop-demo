                    ...
                    <div key={item.id} className="group bg-white rounded-2xl shadow-lg shadow-amber-200/20 hover:shadow-2xl hover:shadow-amber-300/30 transition-all duration-500 overflow-hidden border-2 border-amber-100 hover:border-amber-300">
                      <div className="flex flex-col lg:flex-row">
                        <div className="relative w-32 lg:w-40 aspect-square overflow-hidden bg-amber-50">
                          ...
                          {item.sale_price && parseFloat(String(item.sale_price)) > 0 && (
                            <div className="absolute top-4 left-4 bg-red-500 text-white px-4 py-2 text-sm font-bold rounded-xl shadow-lg text-center min-w-[70px]">
                        ...
                        <div className="flex-1 p-4 flex flex-col justify-between">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t border-amber-100">
                              ...
                              {item.sale_price && parseFloat(String(item.sale_price)) > 0 && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)) ? (
                                <>
                                  <span className="text-lg font-bold text-amber-800">
                                    {formatCurrency(item.sale_price as any, storeSettings, currencies)}
                                  </span>
                                  <span className="text-base text-slate-400 line-through">
                                    {formatCurrency(item.price as any, storeSettings, currencies)}
                                  </span>
                            ...
                            <AddToCartButton
                              product={item as any}
                              store={store}
                              className={`inline-flex items-center gap-2 bg-amber-700 text-white px-6 py-3 rounded-2xl font-bold hover:bg-amber-800 transition-all duration-300 shadow-xl hover:shadow-2xl transform hover:-translate-y-1 ${item.stock <= 0 ? 'cursor-not-allowed opacity-50 transform-none' : ''}`}
                            />
                          </div>
                        </div>
                      </div>
