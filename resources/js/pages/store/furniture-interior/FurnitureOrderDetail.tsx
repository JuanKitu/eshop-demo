  ...
  const renderVariants = (item: any) => {
    ...
    if (!variants || typeof variants !== 'object' || Object.keys(variants).length === 0) {
      return item.variant_name ? (
        <div className="text-xs text-amber-800 bg-amber-100 px-2 py-0.5 rounded inline-block">
          {item.variant_name}
        </div>
      ) : null;
    }
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-1">
        {Object.entries(variants).map(([key, value]: [string, any]) => (
          <span key={key} className="text-[10px] text-amber-800 bg-amber-100 px-2 py-0.5 rounded font-medium">
            {key}: {value}
          </span>
        ))}
      </div>
                        ...
                        <tbody className="bg-white">
                          {order.items.map((item, index) => (
                            <tr key={item.id} className={index % 2 === 0 ? 'bg-amber-25' : 'bg-white'}>
                              ...
                              <td className="p-4 text-right font-bold text-slate-900">{formatCurrency(item.price as any, storeSettings, currencies)}</td>
                              <td className="p-4 text-right font-bold text-amber-800">{formatCurrency((item.price * item.quantity) as any, storeSettings, currencies)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                      ...
                      <div className="bg-amber-50 rounded-2xl p-6 border-2 border-amber-200">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between py-2">
                            ...
                            <span className="font-bold text-slate-900">{formatCurrency(parseFloat(String(order.subtotal)) as any, storeSettings, currencies)}</span>
                          </div>
                          {parseFloat(String(order.discount)) > 0 && (
                            <div className="flex justify-between py-2">
                              <span className="text-green-600">Discount:</span>
                              <span className="font-bold text-green-600">-{formatCurrency(parseFloat(String(order.discount)) as any, storeSettings, currencies)}</span>
                            </div>
                          )}
                          <div className="flex justify-between py-2">
                            <span className="text-slate-600">Delivery:</span>
                            <span className="font-bold text-slate-900">{formatCurrency(parseFloat(String(order.shipping || 0)) as any, storeSettings, currencies)}</span>
                          </div>
                          <div className="flex justify-between py-2">
                            <span className="text-slate-600">Tax:</span>
                            <span className="font-bold text-slate-900">{formatCurrency(parseFloat(String(order.tax || 0)) as any, storeSettings, currencies)}</span>
                          </div>
                          <div className="border-t-3 border-amber-300 pt-3">
                            <div className="flex justify-between">
                              <span className="text-2xl font-bold text-amber-800">TOTAL:</span>
                              <span className="text-2xl font-bold text-amber-800">{formatCurrency(parseFloat(String(order.total)) as any, storeSettings, currencies)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
