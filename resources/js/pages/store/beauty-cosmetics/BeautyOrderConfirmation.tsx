...
export default function BeautyOrderConfirmation({
  ...
  const storeSlug = (props as any).store?.slug || store.slug || 'demo';
  ...
  const orderData = (order || defaultOrder) as any;
                  ...
                  <div className="mb-8">
                        ...
                        return (
                          <div key={item.id} className="flex items-center space-x-4 p-4 bg-rose-50 rounded-2xl">
                            ...
                            <div className="text-right">
                              <p className="font-semibold text-rose-600">{formatCurrency(itemTotal, storeSettings as any, currencies as any)}</p>
                              <p className="text-sm text-gray-500">{formatCurrency(item.price, storeSettings as any, currencies as any)} each</p>
                            </div>
                          </div>
                        );
                      })}
                    ...
                    <div className="mt-8 bg-rose-50 rounded-2xl p-6">
                      <div className="space-y-3">
                        {(orderData.subtotal ?? 0) > 0 && (orderData.discount ?? 0) > 0 && (
                          <div className="flex justify-between text-gray-600">
                            <span>Subtotal</span>
                            <span>{formatCurrency(orderData.subtotal || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        )}
                        {(orderData.discount ?? 0) > 0 && (
                          <div className="flex justify-between text-green-600">
                            <span>Discount {orderData.coupon_code && `(${orderData.coupon_code})`}</span>
                            <span>-{formatCurrency(orderData.discount || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        )}
                        {(orderData.shipping ?? 0) > 0 && (
                          <div className="flex justify-between text-gray-600">
                            <span>Shipping</span>
                            <span>{formatCurrency(orderData.shipping || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        )}
                        {(orderData.tax ?? 0) > 0 && (
                          <div className="flex justify-between text-gray-600">
                            <span>Tax</span>
                            <span>{formatCurrency(orderData.tax || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        )}
                        <div className="border-t border-rose-200 pt-3">
                          <div className="flex justify-between text-xl font-bold text-gray-900">
                            <span>Total</span>
                            <span className="text-rose-600">{formatCurrency(orderData.total, storeSettings as any, currencies as any)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
