                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-6 text-sm text-white/80">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span>Total: {formatCurrency(order.total, storeSettings as any, currencies as any)}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                      ...
                      {order.items.map((item) => (
                        <div key={item.id} className="flex items-center space-x-6 p-6 bg-rose-50 rounded-2xl">
                          ...
                          <div className="flex-1">
                              ...
                              return hasValidVariants ? (
                                <div className="flex flex-wrap gap-2 mt-1 mb-2">
                                  {Object.entries(variants).map(([key, value]) => (
                                    <span key={key} className="text-xs text-rose-500 italic bg-rose-50 px-2.5 py-1 rounded-full border border-rose-100 shadow-sm">
                                      {key}: {String(value)}
                                    </span>
                                  ))}
                                </div>
                            ...
                            <p className="text-gray-600">Qty: {item.quantity} × {formatCurrency(item.price, storeSettings as any, currencies as any)}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-xl font-bold text-rose-600">{formatCurrency(item.price * item.quantity, storeSettings as any, currencies as any)}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      <div className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl p-6">
                        <div className="space-y-4">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-semibold text-gray-900">{formatCurrency(order.subtotal, storeSettings as any, currencies as any)}</span>
                          </div>
                          {order.discount > 0 && (
                            <div className="flex justify-between items-center text-green-600">
                              <span>Discount:</span>
                              <span className="font-semibold">-{formatCurrency(order.discount, storeSettings as any, currencies as any)}</span>
                            </div>
                          )}
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">Shipping:</span>
                            <span className="font-semibold text-gray-900">{formatCurrency(order.shipping, storeSettings as any, currencies as any)}</span>
                          </div>
                          <div className="flex justify-between items-center">
                            <span className="text-gray-600">Tax:</span>
                            <span className="font-semibold text-gray-900">{formatCurrency(order.tax, storeSettings as any, currencies as any)}</span>
                          </div>
                          <div className="border-t border-rose-200 pt-4">
                            <div className="flex justify-between items-center">
                              <span className="text-xl font-bold text-gray-900">Total:</span>
                              <span className="text-xl font-bold text-rose-600">{formatCurrency(order.total, storeSettings as any, currencies as any)}</span>
                            </div>
                          </div>
                        </div>
                      </div>
