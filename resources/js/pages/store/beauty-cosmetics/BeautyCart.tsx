...
function BeautyCartContent({
  ...
  const { items, updateQuantity, removeItem, summary, refreshCart, loading } = useCart();
                      ...
                      <div className="flex items-center space-x-6">
                        ...
                        <div className="flex-1">
                          ...
                          {item.variants && Object.entries(item.variants).length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-2">
                              {Object.entries(item.variants).map(([key, value]) => (
                                <span key={key} className="text-xs text-gray-500 italic bg-rose-50 px-2 py-1 rounded-full">
                                  {key}: {String(value)}
                                </span>
                              ))}
                            </div>
                          ...
                          <div className="flex items-center space-x-2 mb-4">
                            {item.sale_price ? (
                              <>
                                <span className="text-xl font-bold text-rose-600">{formatCurrency(item.sale_price, storeSettings as any, currencies as any)}</span>
                                <span className="text-sm text-gray-400 line-through">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                              </>
                            ) : (
                              <span className="text-xl font-bold text-rose-600">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                            )}
                          </div>
                ...
                <div className="bg-gradient-to-br from-rose-50 to-pink-50 rounded-2xl p-8 shadow-lg sticky top-8">
                  ...
                  <div className="space-y-4 mb-8">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-gray-900">{formatCurrency(dynamicSummary.subtotal, storeSettings as any, currencies as any)}</span>
                    </div>

                    {dynamicSummary.discount > 0 && (
                      <div className="flex justify-between items-center text-green-600">
                        <span>Discount</span>
                        <span className="font-semibold">-{formatCurrency(dynamicSummary.discount, storeSettings as any, currencies as any)}</span>
                      </div>
                    )}
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-semibold text-gray-900">{formatCurrency(dynamicSummary.tax, storeSettings as any, currencies as any)}</span>
                    </div>

                    <div className="border-t border-rose-200 pt-4">
                      <div className="flex justify-between items-center text-xl">
                        <span className="font-bold text-gray-900">Total</span>
                        <span className="font-bold text-rose-600">{formatCurrency(dynamicSummary.total, storeSettings as any, currencies as any)}</span>
                      </div>
                    </div>
                  </div>
