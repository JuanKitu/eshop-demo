...
function ElectronicsCartContent({
  ...
  const dynamicCartItems = items as any[] as CartItem[];
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6">
                      ...
                      <div className="space-y-6">
                        {dynamicCartItems.map((item) => {
                          ...
                          const hasDiscount = item.sale_price && parseFloat(String(item.sale_price)) < parseFloat(String(item.price));
                          const itemPrice = parseFloat(String(hasDiscount ? item.sale_price : item.price || 0));
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="w-20 h-20 flex-shrink-0">
                                <img
                                  src={getImageUrl(product.cover_image as any) || 'https://images.unsplash.com/photo-1572569511254-d8f925fe2cbb?w=80&h=80&fit=crop&crop=center'}
                                  alt={product.name}
                              ...
                              <div className="flex-1">
                                <h3 className="font-bold text-gray-900 mb-1">
                                  <a
                                    href={generateStoreUrl('store.product', store, { id: item.product_id || (product as any).id || 0 })}
                                ...
                                {item.variants && Object.entries(item.variants).length > 0 && (
                                  <div className="flex flex-wrap gap-2 mb-2">
                                    {Object.entries(item.variants).map(([key, value]) => (
                                      <span key={key} className="text-xs text-gray-500 italic bg-gray-100 px-2 py-0.5 rounded">
                                        {key}: {String(value)}
                                      </span>
                                    ))}
                                  </div>
                                ...
                                <div className="flex items-center gap-2">
                                  <span className="text-lg font-bold text-blue-600">
                                    {formatCurrency(itemPrice, storeSettings as any, currencies as any)}
                                  </span>
                                  {hasDiscount && (
                                    <span className="text-sm text-gray-500 line-through">
                                      {formatCurrency(parseFloat(String(item.price || 0)), storeSettings as any, currencies as any)}
                                    </span>
                                  )}
                                </div>
                              </div>
                              ...
                              <div className="text-right">
                                <div className="text-lg font-bold text-gray-900">
                                  {formatCurrency(itemPrice * item.quantity, storeSettings as any, currencies as any)}
                                </div>
                              </div>
                            </div>
                          );
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-6">
                      ...
                      <div className="space-y-4 mb-6">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold">{formatCurrency(dynamicSummary.subtotal || 0, storeSettings as any, currencies as any)}</span>
                        </div>

                        {(dynamicSummary.discount || 0) > 0 && (
                          <div className="flex justify-between text-green-600">
                            <span className="text-gray-600">Discount</span>
                            <span className="font-semibold">-{formatCurrency(dynamicSummary.discount || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        )}
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold">
                            {(dynamicSummary.shipping || 0) === 0 ? 'Free' : formatCurrency(dynamicSummary.shipping || 0, storeSettings as any, currencies as any)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Tax</span>
                          <span className="font-semibold">{formatCurrency(dynamicSummary.tax || 0, storeSettings as any, currencies as any)}</span>
                        </div>
                        <div className="border-t border-gray-200 pt-4">
                          <div className="flex justify-between">
                            <span className="text-lg font-bold text-gray-900">Total</span>
                            <span className="text-lg font-bold text-blue-600">{formatCurrency(dynamicSummary.total || 0, storeSettings as any, currencies as any)}</span>
                          </div>
                        </div>
                      </div>
