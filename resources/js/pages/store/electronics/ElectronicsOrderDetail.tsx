                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between mb-8">
                  <div className="space-y-2">
                    <div className="flex items-center space-x-4">
                      ...
                      <span className={`px-3 py-1 text-xs font-semibold tracking-wider uppercase ${order.status.toLowerCase() === 'delivered' ? 'bg-green-100 text-green-600' :
                    ...
                    {order.items.map((item) => (
                      <div key={item.id} className="flex items-center space-x-4 p-4 border border-gray-200">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">Qty: {item.quantity} × {formatCurrency(item.price, storeSettings as any, currencies as any)}</p>
                        </div>
                        <div className="text-right">
                          <p className="font-bold text-blue-600">{formatCurrency(item.price * item.quantity, storeSettings as any, currencies as any)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="bg-gray-50 p-4 space-y-3">
                      <div className="flex justify-between">
                        <span>Subtotal:</span>
                        <span className="font-semibold">{formatCurrency(order.subtotal, storeSettings as any, currencies as any)}</span>
                      </div>
                      {order.discount > 0 && (
                      ...
                      <div className="flex justify-between">
                        <span>Shipping:</span>
                        <span className="font-semibold">{formatCurrency(order.shipping, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Tax:</span>
                        <span className="font-semibold">{formatCurrency(order.tax, storeSettings as any, currencies as any)}</span>
                      </div>
                      <div className="border-t border-gray-300 pt-3">
                        <div className="flex justify-between">
                          <span className="text-lg font-bold">Total:</span>
                          <span className="text-lg font-bold text-blue-600">{formatCurrency(order.total, storeSettings as any, currencies as any)}</span>
                        </div>
                      </div>
                    </div>
                  </div>
