                          ...
                          <div className="flex-1 p-8">
                            <div className="flex justify-between items-start h-full">
                              <div className="flex-1">
                                ...
                                {((product.average_rating || (product as any).rating || 0) > 0 || (product.total_reviews || (product as any).reviews_count || 0) > 0) && (
                                  <div className="flex items-center mb-4">
                                    ...
                                    <span className="text-sm font-semibold text-slate-600 ml-2">({Number(product.average_rating || (product as any).rating || 0).toFixed(1)})</span>
                                    <span className="text-xs text-blue-600 ml-2 font-medium">• {product.total_reviews || (product as any).reviews_count || 0} reviews</span>
                                  </div>
                                )}
                                <div className="flex items-center mb-4">
                                  {product.sale_price ? (
                                    <>
                                      <span className="text-2xl font-bold text-blue-600">{formatCurrency(product.sale_price, storeSettings as any, currencies as any)}</span>
                                      <span className="text-lg text-gray-500 line-through ml-3">{formatCurrency(product.price, storeSettings as any, currencies as any)}</span>
                                      <span className="ml-3 bg-red-100 text-red-700 text-xs font-bold px-2 py-1 rounded-full">
                                        {Math.round(((parseFloat(String(product.price)) - parseFloat(String(product.sale_price))) / parseFloat(String(product.price))) * 100)}% OFF
                                      </span>
                                    </>
                                  ) : (
                                    <span className="text-2xl font-bold text-slate-800">{formatCurrency(product.price, storeSettings as any, currencies as any)}</span>
                                  )}
                                </div>
                                <div className="flex items-center">
                                  {product.stock > 0 ? (
                    ...
                    {Array.from({ length: Math.min(5, pagination.last_page) }, (_, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`px-3 py-2 border rounded-md text-sm ${page === pagination.current_page
