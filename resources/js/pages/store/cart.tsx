...
export default function Cart({
  ...
  const { props } = usePage();
  const storeSlug = (props as any).store?.slug || (props as any).theme || 'home-accessories';
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {dynamicCartItems.map((item) => {
                          const itemPrice = (item.sale_price && parseFloat(item.sale_price as any) < parseFloat(item.price as any)) ? item.sale_price : item.price;
                          ...
                          return (
                            <tr key={item.id} className="hover:bg-gray-50">
                                ...
                                <div className="flex items-center">
                                  ...
                                  <div className="ml-4">
                                    ...
                                    <div className="md:hidden mt-1">
                                      {item.sale_price && parseFloat(String(item.sale_price)) < parseFloat(String(item.price)) ? (
                                        <div className="flex items-center">
                                          <span className="font-bold text-primary">{formatCurrency(item.sale_price, storeSettings as any, currencies as any)}</span>
                                          <span className="text-gray-500 line-through ml-2 text-xs">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                        </div>
                                      ) : (
                                        <span className="font-bold">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                      )}
                                    </div>
                                    ...
                                    <div className="md:hidden mt-1">
                                      ...
                                      <span className="font-bold">{formatCurrency(itemSubtotal, storeSettings as any, currencies as any)}</span>
                                    </div>
                                  </div>
                                </div>
                              </td>
                              <td className="py-4 px-6 text-center hidden md:table-cell">
                                {item.sale_price && parseFloat(item.sale_price as any) < parseFloat(item.price as any) ? (
                                  <div className="flex items-center justify-center">
                                    <span className="font-bold text-primary">{formatCurrency(item.sale_price, storeSettings as any, currencies as any)}</span>
                                    <span className="text-gray-500 line-through ml-2 text-sm">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                  </div>
                                ) : (
                                  <span className="font-bold">{formatCurrency(item.price, storeSettings as any, currencies as any)}</span>
                                )}
                              ...
                              <td className="py-4 px-6 text-right font-bold hidden md:table-cell">
                                {formatCurrency(itemSubtotal, storeSettings as any, currencies as any)}
                              </td>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 shadow">
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatCurrency(dynamicSummary.subtotal, storeSettings as any, currencies as any)}</span>
                      </div>

                      {dynamicSummary.discount > 0 && (
                        <div className="flex justify-between text-green-600">
                          <span>Discount</span>
                          <span>-{formatCurrency(dynamicSummary.discount, storeSettings as any, currencies as any)}</span>
                        </div>
                      )}

                      <div className="flex justify-between">
                        <span className="text-gray-600">Shipping</span>
                        <span className="font-medium">{formatCurrency(dynamicSummary.shipping, storeSettings as any, currencies as any)}</span>
                      </div>

                      <div className="flex justify-between">
                        <span className="text-gray-600">Tax</span>
                        <span className="font-medium">{formatCurrency(dynamicSummary.tax, storeSettings as any, currencies as any)}</span>
                      </div>

                      <div className="border-t pt-4 mt-4">
                        <div className="flex justify-between">
                          <span className="text-lg font-bold">Total</span>
                          <span className="text-lg font-bold text-primary">{formatCurrency(dynamicSummary.total, storeSettings as any, currencies as any)}</span>
                        </div>
                      </div>
