...
export default function BlogDetail({
  ...
  const [ThemeBlogPost, setThemeBlogPost] = React.useState<React.ComponentType<any> | null>(null);
  const [isLoading, setIsLoading] = React.useState(true);

  React.useEffect(() => {
    const loadThemeBlogPost = async () => {
      if (actualTheme === 'default' || actualTheme === 'home-accessories') {
        setThemeBlogPost(null);
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        switch (actualTheme) {
          case 'fashion':
            blogPostModule = await import('@/pages/store/fashion/FashionBlogPost');
            break;
          case 'electronics':
            blogPostModule = await import('@/pages/store/electronics/ElectronicsBlogPost');
            break;
          case 'beauty-cosmetics':
            blogPostModule = await import('@/pages/store/beauty-cosmetics/BeautyBlogPost');
            break;
          case 'jewelry':
            blogPostModule = await import('@/pages/store/jewelry/JewelryBlogPost');
            break;
          case 'watches':
            blogPostModule = await import('@/pages/store/watches/WatchesBlogPost');
            break;
          case 'furniture-interior':
            blogPostModule = await import('@/pages/store/furniture-interior/FurnitureBlogPost');
            break;
          case 'cars-automotive':
            blogPostModule = await import('@/pages/store/cars-automotive/CarsBlogPost');
            break;
          case 'baby-kids':
            blogPostModule = await import('@/pages/store/baby-kids/BabyKidsBlogPost');
            break;
          case 'perfume-fragrances':
            blogPostModule = await import('@/pages/store/perfume-fragrances/PerfumeBlogPost');
            break;
          default:
            blogPostModule = null;
        }
        ...
        if (blogPostModule) {
          setThemeBlogPost(() => blogPostModule.default);
        } else {
          setThemeBlogPost(null);
        }
      } catch (error) {
        console.error('Error loading theme-specific blog post page:', error);
        setThemeBlogPost(null);
      } finally {
        setIsLoading(false);
      }
  ...
  // If we have a theme-specific blog post, render it
  if (ThemeBlogPost) {
    return (
      <ThemeBlogPost
        post={post || {}}
  ...
  return (
    <>
      <Head>
        <title>{(post?.meta_title || post?.title || 'Blog Post') + ' - ' + (store?.name || 'Store')}</title>
        ...
        {post?.featured_image && <meta property="og:image" content={getImageUrl(post.featured_image)} />}
        {post?.featured_image && <meta name="twitter:image" content={getImageUrl(post.featured_image)} />}
        {post?.index_in_search === false && <meta name="robots" content="noindex,nofollow" />}
      </Head>
