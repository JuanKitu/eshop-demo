...
export default function Plans({ plans: initialPlans, billingCycle: initialBillingCycle = 'monthly', hasDefaultPlan, isAdmin = false, currentPlan, userTrialUsed, paymentMethods = [], activePlansCount = 0 }: Props) {
  ...
  // Use billing cycle passed from backend (which uses user's plan_duration if no URL param)
  ...
  // When currentPlan updates and doesn't match displayed billing cycle,
  // reload page data to get fresh plans with correct billing cycle
  useEffect(() => {
    if (!currentPlan?.duration) return;
    
    const urlParams = new URLSearchParams(window.location.search);
    const urlBillingCycle = urlParams.get('billing_cycle');
    
    // If currentPlan duration differs from what we're showing, reload with correct data
    if (currentPlan.duration !== (urlBillingCycle || billingCycle)) {
      // Use Inertia reload to fetch fresh data without full page refresh
      router.reload({ 
        only: ['plans', 'currentPlan'],
        data: { billing_cycle: currentPlan.duration }
      });
    }
  ...
  const getActionButton = (plan: Plan) => {
    ...
    if (currentPlan && currentPlan.id === plan.id && currentPlan.expires_at && new Date(currentPlan.expires_at) > new Date()) {
      return (
        <Button disabled className="w-full bg-green-100 text-green-800 border-green-200 flex items-center gap-2">
          <Crown className="h-4 w-4" />
          <span>{t('Already Subscribed')}</span>
        </Button>
      );
    }
        ...
        <Button disabled className="w-full flex items-center gap-2">
          ...
          {currentPlan?.duration && (
            <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 capitalize">
              {t(currentPlan.duration.toLowerCase())}
            </Badge>
          )}
        </Button>
      );
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {plans.map((plan) => (
            ...
            >
              ...
              {!isAdmin && plan.is_current && currentPlan && (
        ...
        {!isAdmin && selectedPlan && (
          <PlanSubscriptionModal
            ...
            onSuccess={() => {
              router.reload({ only: ['plans', 'currentPlan'] });
            }}
            plan={selectedPlan}
