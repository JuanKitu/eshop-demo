...
function VariantSelector({ product, open, onClose, onSelectVariant }: VariantSelectorProps) {
  ...
  // Get current variant option based on selected options
  const getCurrentVariantOption = () => {
    if (!product || !product.variant_options || !Array.isArray(product.variant_options)) return null;

    // We need to match the order of `product.variants` to create the combination name
    const comboParts = product.variants.map((v: any) => selectedOptions[v.name]).filter(Boolean);
    const comboName = comboParts.join(' - ');

    return product.variant_options.find((opt: any) => opt.name === comboName) || null;
  };

  const getOriginalPrice = () => {
    const option = getCurrentVariantOption();
    if (option && option.price && parseFloat(option.price) > 0) return option.price;
    return product ? product.price : 0;
  };

  const getSalePrice = () => {
    const option = getCurrentVariantOption();
    if (option && option.sale_price && parseFloat(option.sale_price) > 0) return option.sale_price;
    if (product && product.sale_price && parseFloat(product.sale_price) > 0 && (!option || !option.price)) return product.sale_price;
    return null;
  };

  const getEffectivePrice = () => {
    const salePrice = getSalePrice();
    return salePrice !== null ? salePrice : getOriginalPrice();
  };
  ...
  const handleAddToCart = () => {
    if (hasAttributeVariants()) {
      ...
      const option = getCurrentVariantOption();
      const variant = {
        id: option ? option.id : generateVariantId(),
        name: generateVariantName(),
        price: getEffectivePrice() // Use computed variant price
      };
      ...
      <DialogContent className="sm:max-w-[425px]">
                ...
                <div className="flex flex-col">
                  {getSalePrice() ? (
                    <div className="flex items-center gap-2">
                      <span className="text-lg font-bold text-green-600">{formatCurrency(parseFloat(getSalePrice() as any))}</span>
                      <span className="text-sm text-muted-foreground line-through">{formatCurrency(parseFloat(getOriginalPrice() as any))}</span>
                    </div>
                  ) : (
                    <span className="text-lg font-bold">{formatCurrency(parseFloat(getOriginalPrice() as any))}</span>
                  )}
                  {getCurrentVariantOption() && (
                    <p className={`text-sm mt-1 ${Number(getCurrentVariantOption()?.stock) > 0 ? 'text-green-600' : 'text-red-500'}`}>
                      {Number(getCurrentVariantOption()?.stock) > 0
                        ? t('In Stock: {{count}}', { count: getCurrentVariantOption()?.stock })
                        : t('Out of Stock')}
                    </p>
                  )}
                </div>
        ...
        <div className="flex justify-end space-x-2">
          ...
          <Button
            ...
            disabled={
              (!hasAttributeVariants() && (!Array.isArray(product?.variants) || product?.variants.length === 0)) ||
              (getCurrentVariantOption() && Number(getCurrentVariantOption()?.stock) <= 0)
            }
          >
            {t('Add to Cart')}
          </Button>
