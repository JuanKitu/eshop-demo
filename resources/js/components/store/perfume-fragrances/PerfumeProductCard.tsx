...
export default function PerfumeProductCard({ product, storeSettings, currencies }: PerfumeProductCardProps) {
    ...
    if (product.variants) {
      try {
        ...
        if (variants && typeof variants === 'object') {
          // Check if variant structure is active
  ...
  const { minPrice, maxPrice, minSalePrice, maxSalePrice, hasVariants: calculatedHasVariants } = getProductPrice(product);

  const displayPrice = () => {
    if (calculatedHasVariants && minPrice !== maxPrice) {
        ...
        <div className="space-y-3">
          {hasVariants ? (
            ...
            <button
              ...
              className={`w-full py-3 rounded-full font-medium transition-colors duration-300 ${isOutOfStock
