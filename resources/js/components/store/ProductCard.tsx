...
export default function ProductCard({
  ...
  const isOnSale = sale_price && parseFloat(String(sale_price)) < parseFloat(String(price));
    ...
    >
      ...
      <div className="p-4">
        ...
        <div className="mt-1 flex items-center">
          {isOnSale ? (
            <>
              <span className="font-bold text-primary">{formatCurrency(sale_price || 0, finalStoreSettings as any, finalCurrencies as any)}</span>
              <span className="text-gray-500 text-sm line-through ml-2">{formatCurrency(price, finalStoreSettings as any, finalCurrencies as any)}</span>
            </>
          ) : (
            <span className="font-bold">{formatCurrency(price, finalStoreSettings as any, finalCurrencies as any)}</span>
          )}
        </div>
        ...
        <div className="mt-3">
          <AddToCartButton
            product={{ id, name, price, sale_price: sale_price ?? undefined, cover_image, variants, stock, is_active }}
            store={store}
