...
export default function ElectronicsProductCard({
  ...
  const isOnSale = !!(product.sale_price && parseFloat(String(product.sale_price)) < parseFloat(String(product.price)));
  const isInStock = product.stock > 0 && product.is_active;
  const discountPercentage = isOnSale ? Math.round(((parseFloat(String(product.price)) - parseFloat(String(product.sale_price!))) / parseFloat(String(product.price))) * 100) : 0;
        ...
        <div className="mb-4">
          {isOnSale ? (
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold text-blue-600">{formatCurrency(product.sale_price || 0, finalStoreSettings as any, finalCurrencies as any)}</span>
              <span className="text-lg text-gray-500 line-through">{formatCurrency(product.price, finalStoreSettings as any, finalCurrencies as any)}</span>
            </div>
          ) : (
            <span className="text-2xl font-bold text-gray-900">{formatCurrency(product.price, finalStoreSettings as any, finalCurrencies as any)}</span>
          )}
        </div>
