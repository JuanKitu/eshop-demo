...
const FurnitureProductCard: React.FC<FurnitureProductCardProps> = ({ product, storeSettings, currencies }) => {
  ...
  const formatPrice = (price: any) => {
    return formatCurrency(price, storeSettings, currencies);
  };
    ...
    <div className="group bg-white rounded-3xl shadow-lg shadow-amber-200/30 hover:shadow-2xl hover:shadow-amber-300/40 transition-all duration-500 overflow-hidden border border-amber-100 hover:border-amber-200 transform hover:-translate-y-2">
      ...
      <div className="relative aspect-square overflow-hidden bg-amber-50">
        ...
        <div className="absolute bottom-3 right-3 flex gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-2 group-hover:translate-y-0">
          ...
          <button
            ...
            className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 shadow-lg ${isProductInWishlist
      ...
      <div className="p-5 bg-white">
        ...
        <div className="flex items-center gap-2 mb-4">
          {product.sale_price && parseFloat(String(product.sale_price)) > 0 && parseFloat(String(product.sale_price)) < parseFloat(String(product.price)) ? (
            <>
              ...
              <span className="bg-red-100 text-red-700 text-xs font-bold px-2 py-1 rounded-full ml-auto">
                Save {formatPrice(parseFloat(String(product.price)) - parseFloat(String(product.sale_price)))}
              </span>
            </>
          ) : (
            <span className="text-2xl font-bold text-amber-800">
        ...
        <AddToCartButton
          product={product}
          store={store}
          className={`w-full bg-amber-700 text-white py-3 px-4 rounded-2xl font-bold hover:bg-amber-800 transition-all duration-300 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transform hover:-translate-y-1 ${isOutOfStock ? 'cursor-not-allowed opacity-50' : ''}`}
        />
      </div>
    </div>
  );
